...
describe('Registration', () => {
  ...
  it('returns to the building step with completion field errors under their input', async () => {
    vi.mocked(completeRegistration).mockRejectedValue(new ApiError('Datos inválidos', 422, { name: ['Ya existe un edificio con ese nombre.'] }))
    await renderPage({ ...draft, verified: true })
    const user = userEvent.setup()
    await user.type(screen.getByLabelText('Nombre del edificio'), 'Central')
    await user.type(screen.getByLabelText('Dirección'), 'Av. Central 123')
    ...
    await user.click(screen.getByRole('button', { name: /Continuar/ }))
    await user.click(screen.getByRole('button', { name: 'Comenzar prueba gratis' }))
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Agrega tu primer edificio')
    expect(screen.getByLabelText('Nombre del edificio')).toHaveAccessibleDescription('Ya existe un edificio con ese nombre.')
