...
function ReservationsContent({
  ...
  const weekStart = startOfWeek(search.date ?? today)
  ...
  const amenityOptions = (amenitiesQuery.data?.data ?? []).map((amenity) => ({ value: amenity.id, label: amenity.name }))
    ...
    <div className="@container flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 items-start gap-4 @4xl:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]">
        <div className="flex min-w-0 flex-col gap-2.5">
          {/* The pager belongs to the list, not the page header: the queue
