<template>
  <!--
    ...
    (mockup 21, "Correo de acceso"). Rendered by
    App\Notifications\EmailChangeCodeNotification.
    Blade data: $firstName, $code (6 digits), $minutes (code lifetime)
  -->
  <WasiyLayout>
    <Preheader>Tu código para confirmar el nuevo correo es {{ blade('$code') }}. Vence en {{ blade('$minutes') }} minutos.</Preheader>
    ...
    <Container class="max-w-xl p-0 py-10 sm:py-0">
      ...
      <Section class="rounded-b-lg bg-white px-6 py-12 sm:rounded-none">
        ...
        <Text class="mt-4 text-base text-muted">
          Ingresa este código en Mi cuenta para que este pase a ser tu correo de acceso a Wasiy. Hasta entonces sigues entrando con tu correo actual.
        </Text>

        <Spacer class="h-6" />

        <Section class="rounded-xl bg-cream px-6 py-6 text-center">
          <Text class="m-0 font-sora text-[36px]/10 font-bold tracking-[0.2em] text-teal">
            {{ blade('$code') }}
          </Text>
        </Section>

        <Text class="mt-6 text-sm text-muted">
          Vence en <Text as="span" class="font-semibold text-ink">{{ blade('$minutes') }} minutos</Text>.
          No compartas este código con nadie; el equipo de Wasiy nunca te lo pedirá.
        </Text>

        <Hr class="my-6 bg-border" />

        <Text class="m-0 text-[13px] text-faint">
          Si no pediste este cambio, ignora este correo: tu correo de acceso no cambiará sin el código.
        </Text>
      </Section>
