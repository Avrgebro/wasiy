...
## Revision (2026-09-03) — detail drawer replaces the modal

Mockup 08 was revised alongside Finanzas (mockup 10). List rows and Por aprobar cards open a right-side **drawer** (`reservation-drawer.tsx`) instead of the modal: slot band and status, facts (unit, resident with phone, registered by, decided by, decision note), **Cobros** with the same forward action as Finanzas or a "Ver en Finanzas →" deep link, a **Historial** that merges the booking's activity entries with its movements' (from `GET /reservations/{id}`, which returns `{data, history}`), a derived "Reserva completada" entry drawn hollow, and **Acciones** with one note field: Aprobar / Observar / Rechazar for open requests (note required for the last two), Cancelar reserva behind a confirm step for approved bookings. Front desk sees everything and decides nothing. The `reservation` URL param opens the drawer (deep link from a movement). The "Depósito pendiente" chip and the "Cuotas y depósitos" rail card were dropped from the mockup as redundant with Finanzas. Shared drawer pieces live in `components/ui/detail-drawer-parts.tsx`.

## Revision (2026-09-11) — booking policy simplified (ADR 0041)

The slot-validity rules above (capacity, buffer, min/max duration, advance horizon, per-unit concurrency, cancellation window, and the settings cascade for the last three) were removed. A reservation is now an exclusive booking of one or more consecutive `slot_minutes` slots inside an availability window, gated by `booking_mode`; a fixed 90-day horizon applies to both surfaces; residents cancel until the start, staff any time. The server builds the slot list for both surfaces (`GET /api/amenities/{amenity}/availability` for staff, the portal endpoint unchanged) and the staff drawer no longer enumerates times in the browser. Rationale, numbers and the full decision list live in `docs/adr/0041-reservations-exclusive-slots.md`.
