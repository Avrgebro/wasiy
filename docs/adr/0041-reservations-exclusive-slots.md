...
A reservation is an exclusive booking of an Amenity, for exactly one slot inside its opening hours, gated by a yes-or-no decision when the Amenity requires approval. That is the whole rule set. This ADR supersedes the booking-policy decisions in the M7 roadmap (capacity, buffer, min/max duration, advance horizon, per-unit concurrency, cancellation window, and the settings cascade that governed the last three).
...
- **Slots**: each availability window is cut into consecutive slots of `slot_minutes` from the window start; a tail shorter than a slot is not offered. A booking is exactly one slot: `[start, end)` starts on the slot grid of one availability window for that local weekday and `end == start + slot_minutes` (revised 2026-09-11; runs of consecutive slots were allowed for a day and dropped as needless range logic — an amenity that needs longer bookings gets a longer slot). The server is the only place that computes slots; both surfaces read `GET …/amenities/{amenity}/availability?date=&unit_id=` and post what it offered.
- **Rules on a booking** (`ValidateReservationSlot`): the Amenity is reservable and active; the Unit belongs to the Amenity's Location; `[start, end)` is exactly one slot of one window on that local weekday; start is in the future; no `approved` reservation of the same Amenity overlaps. Only `approved` holds the slot; pending and observed never block.
...
The amenity form loses the Política de reserva section; the booking fields are the switch, the schedule, the slot length and the fees. The operational settings panel loses its Reservas group at both levels. The staff Nueva reserva drawer picks a date on a calendar and one slot from the server's list; the portal does the same on a day strip and a slot grid. The staff reservations page is a day board (Mantine `ResourcesDayView`, static): one row per reservable amenity, bookings as blocks coloured by status, so two blocks in one row show a conflict at a glance; a block opens the detail drawer. The week list, its search, status chips and amenity filter are gone. `@mantine/schedule` pins `core`, `dates` and `hooks` to its exact version, so Mantine upgrades are a lockstep across all installed Mantine packages (9.6.1 as of this revision). Existing reservations are untouched: the migration drops amenity columns only, and the new alignment rule applies to new bookings. The validator drops from thirteen rules to five, and the remaining tests can cover the whole surface, including a daylight-saving timezone.
